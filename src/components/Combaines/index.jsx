...
const index = () => {
  ...
  if (loading) {
    return (
      <div
        role="status"
        className="max-w-screen-sm m-auto w-full p-2 space-y-8 animate-pulse md:space-y-0 md:space-x-8 rtl:space-x-reverse md:flex md:items-center"
      >
          ...
          {products &&
            products.map((product) => (
              <div key={product._id} className="px-1">
                <article className="flex flex-col justify-between bg-white border border-gray-200 rounded-lg shadow-sm md:flex-row hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                  <Link to={`/show/${product._id}`} className="flex-1">
                    <div
                      className="sm:flex-1"
                      style={{
